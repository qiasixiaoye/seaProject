...
| RAG-03 | Chunk boundaries | Fixed-size chunks can split method/result/caption context or mix unrelated sections. | Use structure-first chunking: section heading -> paragraph groups -> fallback token window. Keep captions attached to nearby figure/table references. Use overlap only inside long same-section text. First local pass now chunks Markdown by heading hierarchy and parsed PDFs by page/heading heuristics. MinerU layout blocks remain next. | Chunk text is semantically complete and has `section_path`; no chunk crosses unrelated top-level sections unless explicitly marked as fallback. | P0 |
| RAG-04 | Chunk size | There is no justified default chunk size for Chinese reports, English papers, tables, or data manuals. | Define default profiles: Chinese reports 600-900 chars with 100-150 char overlap; English papers 350-600 tokens with 80-120 token overlap; tables split by logical row groups. First local profiles are documented in `docs/RAG_CHUNKING_STRATEGY.md`. | `docs/EVALUATION_METRICS.md` or RAG setup docs records chunk profiles and rationale. | P1 |
...
| 2026-06-07 | Deterministic bilingual query expansion and multi-route retrieval trace added. Evidence chunks now retain the expanded query, query variant, route hits, and fusion summary. External BM25/RRF and learned rerankers remain future work. | In progress |
...
| 2026-06-07 | Local structure-aware chunking added for Markdown and optionally parsed PDFs. Chunk profiles, parser names, section paths, pages, and content types are exposed in EvidenceChunk and documented in `docs/RAG_CHUNKING_STRATEGY.md`. MinerU/layout-aware figure-table extraction remains future work. | In progress |
